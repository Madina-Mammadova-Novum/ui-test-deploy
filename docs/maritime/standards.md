...
export function calculateFreightRate(params) {
  const { cargoQuantity, route, vesselSpecs, marketConditions, timeOfYear } = params;
...
export function validateStateTransition(currentState, targetState, context) {
  ...
  const missingActions = stateConfig.requiredActions.filter((action) => !completedActions.includes(action));

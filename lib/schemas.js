...
import { checkPhoneValue, formatCurrency } from '@/utils/helpers';
...
export const numberOfUnitsSchema = (maxValue) => {
  return yup.number().typeError('Digits only').min(1).max(maxValue, 'Limit is exceeded');
};
...
export const addressDetailsSchema = (type) => {
  ...
  object[`${type}Province`] = yup.string().nullable();
...
export const companyDetailsSchema = () => ({
  ...
  secondaryPhone: yup
    ...
    .test('secondary-phone-validation', 'Phone number is not valid', checkPhoneValue),
});
...
export const offerSchema = (props = {}) => {
  ...
  return {
    ...
    value: yup
      ...
      .max(
        currentFreight?.ranges?.max?.end || 5,
        `Must be no more than ${
          currentFreight?.value === '$/mt'
            ? `$${formatCurrency(currentFreight?.ranges?.max?.end || 5, true)}`
            : formatCurrency(currentFreight?.ranges?.max?.end || 5)
        }`
      )
      .required(requiredMessage),
    demurrageRate: yup

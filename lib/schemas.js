...
export const toolsSchema = (isFreightCalculator) => {
  const conditionalField = isFreightCalculator ? {
    cargoQuantity: yup.number().typeError(requiredMessage).min(1, 'Value must be greater than 0').required(requiredMessage),
  } : {
    speed: yup.string().nullable(),
  }
  return ({
    calculator: yup.object().nullable().shape({ value: yup.string().required(requiredMessage) }).required(requiredMessage),
    fromPort: yup.object().nullable().shape({ value: yup.string().required(requiredMessage) }).required(requiredMessage),
    toPort: yup.object().nullable().shape({ value: yup.string().required(requiredMessage) }).required(requiredMessage),
    additionalPorts: yup.array().of(
      yup.object().shape({
        port: yup.object().nullable().shape({ value: yup.string().required(requiredMessage) }).required(requiredMessage),
      })
    ),
    ...conditionalField
  })
...
export const tankerDataSchema = () => ({
  ...
  built: yup.number().typeError(yearError).test('len', yearError, val => val && val.toString().length === 4).max(new Date().getFullYear(), 'Please enter a valid four-digit year (YYYY) representing a specific calendar year up to the current year.').required(requiredMessage),
  ...
  loa: yup.string().required(requiredMessage),
  beam: yup.string().required(requiredMessage),
  summerDWT: yup.string().required(requiredMessage),
  summerDraft: yup.string().required(requiredMessage),
  normalBallastDWT: yup.string().required(requiredMessage),
  normalBallastDraft: yup.string().required(requiredMessage),
  cubicCapacity: yup.string().required(requiredMessage),
  ...
  disponentOwner: yup.string().required(requiredMessage),
  disponentOwnerCountry: yup.object().shape({ value: yup.string().required(requiredMessage) }),
});

...
export const personalDetailsSchema = () => ({
  ...
  secondaryPhoneNumber: phoneSchema(),
});
...
export const tankerSlotsDetailsSchema = () => {
  return {
    ...
    applySlots: yup.bool().required(requiredMessage),
...
export const termsAndConditionsSchema = () => {
  return {
    agreedRules: yup.bool().oneOf([true, requiredMessage]),
  };
};
...
export const searchForTankerSchema = () => ({
  ...
  loadPort: yup.object().shape({ value: yup.string().required(requiredMessage) }),
  loadTerminal: yup.object().shape({ value: yup.string().required(requiredMessage) }),
  dischargePort: yup.object().shape({ value: yup.string().required(requiredMessage) }),
  dischargeTerminal: yup.object().shape({ value: yup.string().required(requiredMessage) }),
  cargoType: yup.object().shape({ value: yup.string().required(requiredMessage) }),
  products: yup.array().of(
    yup.object().shape({
      ...
      tolerance: yup.string().required(requiredMessage),
    })
  ),
});

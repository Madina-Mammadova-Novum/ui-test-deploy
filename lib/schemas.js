...
export const searchForTankerSchema = () => ({
  ...
  additionalDischargeOptions: yup.object().shape({
    isAllSelected: yup.boolean(),
    selected: yup.array(),
  }),
  sanctionedCountries: yup.array(),
  excludedInternationallySanctioned: yup.boolean(),
});
  ...
  return {
    ...
    value: yup
      ...
      .min(
        ...
        `Must be greater than ${(currentFreight?.ranges?.min?.start - 0.001 || 2).toFixed(3)}`
      )

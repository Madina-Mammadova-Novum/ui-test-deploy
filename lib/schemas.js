...
yup.addMethod(yup.string, 'validateLatinCharacters', function (settings) {
  return this.test('check-for-latin-characters', 'Please, use latin characters only', function (value) {
    ...
    const isLatin = value.match(REGEX.LATIN_CHARACTERS);
    return isLatin;
  });
});
...
export const addressDetailsSchema = (type) => {
  ...
  object[`${type}Address`] = yup.string().required(requiredMessage).validateLatinCharacters();
  object[`${type}Province`] = yup.string().validateLatinCharacters({ optional: true });
  object[`${type}PostalCode`] = yup.string();
  object[`${type}Address2`] = yup.string().validateLatinCharacters({ optional: true });
...
export const companyDetailsSchema = () => ({
  companyName: yup.string().required(requiredMessage).validateLatinCharacters(),
...
export const uploadSchema = () => ({
  ...
  comment: yup.string().validateLatinCharacters({ optional: true }),
...
export const offerSchema = () => ({
  ...
  comment: yup.string().validateLatinCharacters({ optional: true }),
  value: yup.number().typeError('Digits only').min(1, 'Must be greater than 1').required(),
  demurrageRate: yup.number().typeError('Digits only').min(1, 'Must be greater than 1').required(requiredMessage),
  layTime: yup
    .number()
    .typeError('Digits only')
    .min(1, 'Must be greater than 1')
    .max(100, 'Must be no more than 100 hrs')
    .required(requiredMessage),
...
export const tankerDataSchema = () => ({
  ...
  grades: yup
    ...
    .required(requiredMessage),
  registeredOwner: yup.string().required(requiredMessage).validateLatinCharacters(),

...
export const addressDetailsSchema = (type) => {
  ...
  object[`${type}Province`] = yup.string().nullable().validateLatinCharacters({ optional: true });
  object[`${type}PostalCode`] = yup.string().nullable();
  object[`${type}Address2`] = yup.string().nullable().validateLatinCharacters({ optional: true, includeNumbers: true });

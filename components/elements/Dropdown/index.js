...
const Dropdown = ({ onChange, label, id }) => {
    ...
    return (
        <div className='max-w-[296px]'>
            <label htmlFor={id} className='text-[12px] text-gray font-semibold uppercase'>{label}</label>
            <Select
                id={id}
                options={dropdownOptions}
                closeMenuOnSelect={false}
                onChange={(option) => handleChange(option)}
                formatOptionLabel={({ countryFlag, value }) => (
                    <OptionRow countryFlag={countryFlag} value={value}/>
                )}
                styles={{
                    option: base => ({
                        ...base,
